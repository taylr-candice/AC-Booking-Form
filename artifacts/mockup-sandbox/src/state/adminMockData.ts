...
/**
 * Canonical service-timeline entry stamped on a booking when ops bulk-
 * marks it as chased from the Awaiting-coordination queue. The per-
 * booking "Mark as chased" affordance has been replaced by the
 * structured `logCall` / `logEmail` actions on BookingDetail, but the
 * bulk action still stamps this generic "Marked as chased" entry so
 * ops can fast-track several rows in one go. Used by
 * {@link applyBulkChase}. Pure / data-only.
 */
export function buildChasedTimelineEntry(
  by: string = ADMIN_USER_LABEL,
  at: string = "Just now",
): TimelineEntry {
  return {
...
/**
 * Apply the bulk "Mark as chased" patch to a list of bookings. For every
 * booking whose id is in `ids` we stamp `lastContactedAt` with `nowIso`
 * and append a {@link buildChasedTimelineEntry} marker so the audit
 * trail shows who chased and when. The live demo row (`bk-live`) is
 * silently skipped — same guard as the rest of the admin mutations,
 * since the customer flow is the source of truth there. Bookings not
 * in `ids` pass through untouched (referentially identical), so React
 * re-renders only the rows that actually changed. Pure / data-only.
 */
export function applyBulkChase(
  bookings: readonly AdminBooking[],
  ids: readonly string[],
  nowIso: string,
): AdminBooking[] {
  if (ids.length === 0) return bookings.slice();
  ...
  return bookings.map((b) => {
    ...
    return {
      ...
      serviceTimeline: [...b.serviceTimeline, buildChasedTimelineEntry()],
    };
  });
}
