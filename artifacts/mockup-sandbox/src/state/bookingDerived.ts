 ...
 * cause Step 6 (Schedule) to be skipped, so the visible-step list goes
 * 1-2-3-4-5-7 and the progress indicator shows "Step X of 6".
 */
...
const ALL_STEPS: readonly StepId[] = [1, 2, 3, 4, 5, 6, 7];
...
export function visibleSteps(s: Pick<BookingState, "access_method">): StepId[] {
  if (isCoordinationFlow(s)) {
    return ALL_STEPS.filter((id) => id !== 6);
  }
  return [...ALL_STEPS];
}

/** Total steps to display in the progress indicator (6 or 7). */
...
export function visibleIndex(
  ...
  // If `step` isn't visible (e.g. step 6 in a coordination flow), fall back
  // to the position of step 7 — the place the user would actually be.
 ...
 *
 * If `current` is itself hidden (e.g. step 6 in a coordination flow that
