 ...
 *
 * Background (Task #46):
 *   When a customer taps "Update AC info" from the slot picker (Step 5),
 *   the wrapper's click bridge does two things atomically:
 *     1. Stashes a `return_to` hint pointing back to Step 5 (via
 *        `bookingActions.setReturnTo(5)`).
 *     2. Jumps straight to Step 3 with the contextual origin recorded
 *        (via `bookingActions.editAcFromSlotPicker()`).
 *   Then, when the customer hits Continue on Step 3 (AC), the wrapper
 *   short-circuits the normal `nextStepId` walk and flings them straight
 *   back to the hinted return step (5) instead of walking them through
 *   Property access (Step 4).
 *
...
describe.each(VARIANTS)(
  ...
  ({ Wrapper }) => {
    it(
      "stashes return_to=5 on 'Edit AC info', then short-circuits Continue " +
        "on the AC step straight back to the slot picker (skipping Step 4)",
      async () => {
        ...
        // ── Step 5: slot picker iframe + click "Edit AC info" ───────
        ...
        //   - called editAcFromSlotPicker, which jumped to Step 3 and
        ...
        // ── Step 3: AC iframe + click "Continue" ────────────────────
        ...
        // ── Final assertion: short-circuit back to Step 5, NOT 4 ────
        // The wrapper saw `return_to=5` + `current_step=3` and called
        // `goToStep(5)` instead of walking forward through `nextStepId`
        // (which would have landed on Step 4 — Property access). The
        ...
        // the slot-picker iframe (not Step 4's Property-access
        // iframe). A regression that skipped the short-circuit and
        // ran `nextStepId` instead would render `flow-iframe-4` here.
