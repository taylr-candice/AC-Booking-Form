...
- Mockups live under `src/components/mockups/`:
  ...
  - `admin/AdminApp.tsx` — single-page admin ops mockup (bookings list + detail, slot calendar, units, agents, payments). Reads the customer's current sessionStorage booking via `liveBookingFromSession()` so the demo's live row appears alongside seeded data. Slot calendar supports two scheduling modes per window — `time_based` (window has minutes; bookings consume minutes by service length) and `count_based` (window has N slots; each booking takes one). See `slotIsAvailable()` in `state/adminMockData.ts`. Customers always see only "available / full".
    - **Service catalogue** (Task #182) — Sidebar entry "Services" → `admin/ServicesView.tsx` lists every `AdminService` (split / multi / ducted / null=other) with editable name, base minutes, add-on rule (`per_system | per_extra_unit`), add-on minutes, base price, add-on price. Catalogue lives in AdminApp state (`services` / `commitServices`) and is exposed to derived selectors via `setLiveServiceCatalogueSource` in `state/bookingDerived.ts`. **Outdoor unit placement** — `BuildingDetail` has an `in_property | rooftop` toggle (`AdminBuilding.outdoorPlacement`) plus a `Rooftop overhead minutes` numeric input (`AdminBuilding.rooftopOverheadMinutes`, default `DEFAULT_ROOFTOP_OVERHEAD_MINUTES = 10`). Per-unit override lives in `UnitsView` UnitEditor as a "Different outdoor placement" toggle (`AdminUnit.outdoorPlacementOverride`), mirroring the `overrideType` AC-type override pattern. Helpers `getServiceRuleForAcType`, `getEffectivePlacementForUnit`, `getRecordedAcTypeForUnit` in `adminMockData.ts`. `getBookingDurationMinutes` (in `state/bookingDerived.ts`) is now a pure function — it reads catalogue rules + per-unit placement context via registered resolvers (`setServiceRuleResolver`, `setUnitDurationContextResolver`) instead of hardcoded constants. Defaults preserved: 45m base, 15m add-on, +10m rooftop overhead per system.
...
- Evening windows (5pm–8pm, 180m, `EVENING_WINDOW_MIN`) are supported in the data model (`adminMockData.ts`: `RolloutSlot.window`, `RolloutDay.evening?`, `WINDOW_TIME_RANGE.evening`, `AdminBooking.serviceSlot`, `UnitUnavailableBlocker.slot`, `BuildingRolloutSummary.nextScheduled.slot`, `AdminCreatedScheduleChoice.window`). RL_ASPEN_DAYS seeds 4 evening windows for the demo. Customer pickers render `day.evening` when present. **Admin scheduling paths now accept and surface `"evening"` end-to-end (Task #187):** `NewBookingFlow.pickedWindow`, `AdminApp.appendBooking` / `rescheduleAppointment` / `scheduleCoordinationBooking` / `undoCancelBookingAndReschedule`, `RolloutScheduleEditor`, `SchedulingModal` (formerly ScheduleCoordinationModal), and `rolloutSlotPicker` all type their window param as `"morning" | "afternoon" | "evening"`, render Evening as a third slot choice in the shared `RolloutDayCell` whenever `day.evening` is defined, and stamp Evening on timeline labels via `convertCoordinationToScheduledPatch` / `buildRescheduledTimelineEntry`. `RolloutScheduleEditor` lets admins add an Evening window per day via `addRolloutEveningWindow` (with toast undo via `removeRolloutEveningWindow`).
