...
- Slot picker pages (`booking-slots/SlotsDesktop`, `SlotsMobile`, `SlotsMobileLite`) show a single "Heads up" access-window banner only — the longer accountability nudge that used to differ for agents vs customers has been removed. The banner's footer link now reads **Change access option** (testid `button-edit-access`) and jumps the customer back to Step 4 (Property access) so they can swap to a hands-off option (parcel locker, leave a key, coordinate with tenant) and not have to be home for the entire window. The desktop variant's timezone pill mirrors the city the building is in (`unitCity()` in `bookingHelpers.ts` maps state codes ACT/NSW/VIC/QLD/WA/SA/TAS/NT to Canberra/Sydney/Melbourne/Brisbane/Perth/Adelaide/Hobart/Darwin) instead of always saying "Sydney Time". The booking-flow wrappers' `NAV_GOTO_RETURN_TO` set covers both Step 3 (after edit-AC) and Step 4 (after edit-access) so the next "Continue" tap on either step flings the customer straight back to the slot picker.
